    ...
    <div className="bg-surface-bright selection:bg-primary-container selection:text-primary min-h-screen flex flex-col font-body-md">
      ...
      <main className="flex-1 w-full max-w-[1280px] mx-auto px-4 md:px-10 py-8">
        ...
        <div className="text-sm text-on-surface/60 mb-6 flex items-center gap-2">
          ...
          {selectedCategory ? (
            <>
              <Link to="/sanpham" onClick={() => setSelectedCategory(null)} className="hover:text-primary transition-colors">Sản Phẩm</Link>
              <MdKeyboardArrowRight className="w-4 h-4" />
              <span className="text-on-background font-semibold">{activeCategoryName || 'Danh mục'}</span>
            </>
          ) : (
            <span className="text-on-background font-semibold">Sản Phẩm</span>
          )}
        </div>

        {/* Tiêu đề trang và hiển thị số lượng sản phẩm lọc được */}
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-display-lg font-bold text-on-background mb-2">Sản Phẩm </h1>
          <p className="text-on-surface/60 text-sm">({filteredProducts.length} sản phẩm)</p>
        </div>
